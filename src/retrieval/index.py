...
class LocalEmbeddingIndex:
    ...
    def build(
        ...
        stale_ids = set(collection.get(include=[])["ids"]) - set(new_ids)
        if stale_ids:
    ...
    def build_from_clean(self) -> "LocalEmbeddingIndex":
        """Build this collection from the configured clean JSON artifact."""
        built = type(self).build(
            pd.read_json(self.settings.paths.clean_json),
            self.settings,
    ...
    def search(self, query: str, top_k: int | None = None) -> list[SearchResult]:
        if self.collection is None:
            raise RuntimeError("Collection is not built. Call build_from_clean() first.")
